...
export default function Dashboard() {
  ...
  // ── Lock state (render-time derived, no effects) ───────────────────
  // `decided` is computed once when hydrated; `userUnlocked` is set by
  // onComplete / onUnlock / onClose callbacks.
  type LockView = "setup" | "unlock" | "unlocked";
  const [decided, setDecided] = useState<LockView | null>(null);
  const [userUnlocked, setUserUnlocked] = useState(false);
  const [sawPasscode, setSawPasscode] = useState(false);

  // Render-time lock decision (React "adjust state when props change" pattern).
  if (hasPasscode && !sawPasscode) setSawPasscode(true);
  if (hydrated && decided === null) {
    let next: LockView | null = null;
    try {
      if (sessionStorage.getItem("venting-just-onboarded") === "1") {
        ...
        next = "unlock";
      } else if (sawPasscode && safeSessionGetItem(LOCK_DISMISSED_KEY) !== "1") {
        next = "setup"; // passcode existed but vanished — re-prompt setup
      } else if (safeSessionGetItem(LOCK_DISMISSED_KEY) === "1") {
        next = "unlocked";
      }
      // else next stays null — kv may not have loaded yet
    } catch {
      next = "unlocked";
    }
    if (next !== null) setDecided(next);
  }
  const lock: LockView = userUnlocked
    ? "unlocked"
    : decided === null ? "unlocked"
    : decided === "unlock" && !hasPasscode && sawPasscode ? "unlocked" // passcode vanished
    : decided;
  ...
  if (lock === "unlock") {
    if (!passcodeHash || !passcodeSalt) {
      ...
      // or kv hasn't finished loading. Show a gentle loading state.
      return (
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {hasPasscode && (
                <button
                  type="button"
                  onClick={() => { setDecided("unlock"); setUserUnlocked(false); }}
